...
def test_a_named_artifact_that_does_not_resolve_fails_closed(tmp_path):
    """A record that names its capture is answered by that capture or by nothing.

    Falling back to discovery here is how a damaged history line acquires another run's
    evidence — the failure the recorded name exists to prevent.
    """
    _artifacts(
        tmp_path,
        read_at="2026-08-08T03:17:00+00:00",
        verification_artifact="examples/verification/nightly-commerce-does-not-exist.txt",
    )
    (tmp_path / "examples" / "verification" / "nightly-commerce-2026-08-08T14-31-52.txt").write_text(
        "# generated by ops/nightly-read.sh at 2026-08-08T14:31:52Z\nprecision 1.00\n"
    )
...
def test_a_capture_written_after_the_run_is_not_discovered_for_it(tmp_path):
    """Offset and Z forms of the same instant must compare as instants, not as text."""
    _artifacts(tmp_path, read_at="2026-08-08T03:17:00+00:00")
    verification = tmp_path / "examples" / "verification"
    (verification / "nightly-commerce-2026-08-08.txt").write_text(
        "# generated by ops/nightly-read.sh at 2026-08-08T03:17:00+00:00\nprecision 0.69\n"
    )
    (verification / "nightly-commerce-2026-08-08T14-31-52.txt").write_text(
        "# generated by ops/nightly-read.sh at 2026-08-08T14:31:52Z\nprecision 1.00\n"
    )
    ...
    body = (tmp_path / "reports" / "nightly" / "2026-08-08" / "commerce" / "verification.md").read_text()
    assert "precision 0.69" in body
    assert "precision 1.00" not in body, "a later capture cannot be an earlier run's evidence"
